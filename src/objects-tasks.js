...
function shallowCopy(obj) {
  const newObject = { ...obj };
  const newNewObject = Object.assign(newObject, {});
  return newNewObject;
}
...
function mergeObjects(objects) {
  const array = [];
  objects.forEach((obj) => array.push(...Object.entries(obj)));
...
function makeWord(lettersObject) {
  const arr = [];
  let rememberLetter = '';
  Object.entries(lettersObject).forEach((entries) => {
    entries.forEach((entrie, index) => {
      if (index === 0) {
        rememberLetter = entrie;
      } else {
        entrie.forEach((item) => {
          arr[item] = rememberLetter;
        });
      }
    });
  });
...
function sellTickets(queue) {
  ...
  return newArray.every((item) => item === 1);
}
...
function sortCitiesArray(arr) {
  return arr.sort((a, b) => {
    const compare = a.country.localeCompare(b.country);
    return compare !== 0 ? compare : a.city.localeCompare(b.city);
  });
}
